...
import { Customer, Incident, Invoice, User, WorkSiteAndRequestAPI } from './Model'
...
class MainApi extends AbstractApi {
    ...
    public async putIncidentForWorksite(id: string, incident: Incident): Promise<Incident> {
        try {
            const response = await this.service.get(`/api/worksites/${id}/incident`, {
                params: {
    ...
    public async putEvidenceForIncident(id: string, evidence: string): Promise<Incident> {
        try {
            const response = await this.service.get(`/api/worksites/incident/${id}/evidences`, {
                params: {
                    evidence: evidence,
                }
            })
    ...
    public async putInvoicesForWorksite(id: string, invoice: Invoice): Promise<Invoice> {
        try {
            const response = await this.service.get(`/api/worksites/${id}/invoice`, {
                params: {
                    invoice: {
                        title: invoice.title,
